 ...
 int main (int argc, char * argv[])
{
    ...
    printf("File size: %u bytes\t",
          (uint32_t) st.stx_size);
    printf("Blocks allocated: %u\t",
          (uint32_t) st.stx_blocks);
    printf("Preferred I/O block size: %u bytes\t",
          (uint32_t) st.stx_blksize);

	printf("%s", mode(st.stx_mode));


	printf("ID of containing device: %u, %u \t", 
		  (uint32_t) (major(st.stx_dev_major)) , (uint32_t) minor(st.stx_dev_minor));
	...
	printf("Uid: (%u/\t%s)\tGid: (%u/\t%s)\n",    (uint32_t) st.stx_uid, UserName(st.stx_uid), (uint32_t) st.stx_gid, GroupName(st.stx_gid));
